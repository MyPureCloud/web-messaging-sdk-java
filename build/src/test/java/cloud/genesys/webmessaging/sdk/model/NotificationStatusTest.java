package cloud.genesys.webmessaging.sdk.model;

import com.fasterxml.jackson.databind.ObjectMapper;
import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.*;

/**
 * Tests for NotificationStatus enum.
 */
public class NotificationStatusTest {

    private final ObjectMapper objectMapper = new ObjectMapper();

    @Test
    public void testFromStringNull() {
        assertNull(NotificationStatus.fromString(null));
    }

    @Test
    public void testFromStringUnknownReturnsFallback() {
        NotificationStatus result = NotificationStatus.fromString("NonExistentValue_XYZ");
        assertEquals(NotificationStatus.values()[0], result);
    }

    @Test
    public void testAllValuesHaveNonNullToString() {
        for (NotificationStatus value : NotificationStatus.values()) {
            assertNotNull(value.toString(), "toString() should not be null for " + value.name());
            assertFalse(value.toString().isEmpty(), "toString() should not be empty for " + value.name());
        }
    }

    @Test
    public void testFromStringCaseInsensitive() {
        for (NotificationStatus value : NotificationStatus.values()) {
            assertEquals(value, NotificationStatus.fromString(value.toString().toLowerCase()),
                    "fromString should be case-insensitive for " + value.name());
        }
    }

    @Test
    public void testJsonRoundTrip() throws Exception {
        for (NotificationStatus value : NotificationStatus.values()) {
            String json = objectMapper.writeValueAsString(value);
            NotificationStatus deserialized = objectMapper.readValue(json, NotificationStatus.class);
            assertEquals(value, deserialized, "Round-trip failed for " + value.name());
        }
    }

    @Test
    public void testJsonDeserializationUnknownValue() throws Exception {
        NotificationStatus result = objectMapper.readValue("\"SomethingUnknown_XYZ\"", NotificationStatus.class);
        assertEquals(NotificationStatus.values()[0], result);
    }
}
